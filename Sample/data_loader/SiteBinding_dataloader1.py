...
class ForecastDataset(torch_data.Dataset):
    def __init__(self, df):
        ...
        self.motifs = [r"GGAC", r"GAC", r"AGAC", r"AC"]
    ...
    def __getitem__(self, index):
        ...
        if len(self.data) > 4:
            ...
            AllRegionIndex = self._infer_region_index(onelabels, graph)

        # 结合位点、时序训练数据
        graph, Loop_graph, labels, onehots, onehot_feature, RegionIndex = self.get_data(graph, Loop_graph, onelabels,
                                                                                        oneFeature,
                                                                                        AllRegionIndex)  # 分别为结构图，序列标签，标签图，标签图节点初始特征 (onehot+理化性质)，结构图节点初始特征
    ...
    def _infer_sequence_codes(self, onelabels):
        flat_feature = np.asarray(onelabels[:-1], dtype=np.float64)
        if flat_feature.size < self.size * 4:
            return np.full(self.size, 1, dtype=int)

        try:
            feature_matrix = flat_feature.reshape(self.size, -1)
            if feature_matrix.shape[1] >= 4:
                return np.argmax(feature_matrix[:, :4], axis=1).astype(int) + 1
        except ValueError:
            pass

        return np.full(self.size, 1, dtype=int)

    def _infer_region_index(self, onelabels, graph):
        sequence_codes = self._infer_sequence_codes(onelabels)
        try:
            region_index = self.identify_key_regions(sequence_codes, graph, self.motifs)
            return np.asarray(region_index, dtype=int)
        except Exception:
            start = max(0, (self.size - self.target_length) // 2)
            end = min(self.size - 1, start + self.target_length - 1)
            return np.asarray([start, end], dtype=int)
    ...
    def detect_secondary_structure(self, structure_matrix):
        structure_matrix_np = np.asarray(structure_matrix)
