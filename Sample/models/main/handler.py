...
def load_model(model_dir, fold=1, map_location="cpu"):
    if not model_dir:
        return
    file_name = os.path.join(model_dir, f'{fold}_best_IBGGN.pt')
    print(file_name)
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)
...
def validate_inference_binding_site(model,dataloader):
    ...
    with torch.no_grad():
        ...
        for i, (graph,Loop_graph, labels, onehots, onehot_feature,RegionIndex) in enumerate(dataloader):
            ...
            graph = graph.to(model_device)  # 二级结构图
            Loop_graph = Loop_graph.to(model_device)  # 二级结构图
            labels = labels.to(model_device) # 标签
            onehot_x = onehots.to(model_device)  # RNA_OP
            onehot_x = onehot_x.reshape(-1, 41, 15)
            onehot_feature = onehot_feature.to(model_device)  # 输入结合位点
            RegionIndex=RegionIndex.to(model_device)
...
def train(train_data, valid_data, args, result_file, fold, species):
    ...
    if getattr(args, "batch_size1", None) is None or args.batch_size1 <= 0:
        args.batch_size1 = max(1, min(args.batch_size, len(train_data[0]) - 1))

    ISGNN = ConsensusMotifNetwork(batch=args.batch_size, batch1=args.batch_size1, size=args.size,
                                           num=args.num, device=args.device)  # 输入三个参数
    ...
    while train_batch_size > 1 and len(train_set) % train_batch_size == 1:
        train_batch_size -= 1

    valid_batch_size = min(args.batch_size, len(valid_set))
    train_loader = torch_data.DataLoader(train_set, batch_size=train_batch_size, drop_last=False, shuffle=False,
                                         num_workers=0)
    valid_loader = torch_data.DataLoader(valid_set, batch_size=valid_batch_size, shuffle=False, num_workers=0)
    ...
    for epoch in range(args.epoch):
        ...
        for i, (graph, Loop_graph, labels, onehots, onehot_feature,RegionIndex) in enumerate(train_loader):
            ...
            # print(RegionIndex.shape) #torch.Size([706, 2])
            # print(RegionIndex[0]) #tensor([ 8, 15], device='cuda:0', dtype=torch.int32)

            final_representation, labels_pred, con_loss, distance_Loss = ISGNN(graph,Loop_graph,onehot_x, Elom4_feature, RegionIndex)  # 返回最终潜在表示，返回一个方差损失，再返回一个重构损失
    ...
    # StorFile(best_Real_Predition, './Pre-Encoding/data/' + str(species) + '/Result/Real_Prediction' + str(fold) + '.csv')
    #
    # StorFile(best_Real_Predition_Prob, './Pre-Encoding/data/' + str(species) + '/Result/Real_Prediction_prob' + str(fold) + '.csv')
...
def inverse_validate_process(args, result_train_file, x1, x2, pre1, pre2):  # 五个参数分别为：模型、输出的两种表征、输入的两种表征
    model = load_model(result_train_file, map_location=args.device)
...
def test(test_data, args, result_train_file, species, fold):  #
    ...
    model = load_model(result_train_file, fold=fold, map_location=args.device)
