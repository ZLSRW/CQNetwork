...
class ConsensusMotifNetwork(nn.Module):  # 当前为一层的卷积过程,默认均为三维张量
    def __init__(self, num, size, batch, batch1, device=None):
        ...
        self.NodeConsensusVector = NodeConsensusVector(device=self.device)  # 获得共识图
    ...
    def largest_lamb(self, L, k=1, default_value=1.0):
        """Return the largest Laplacian eigenvalue for each dense graph in the batch."""

        L = (L + L.permute(0, 2, 1)) / 2.0
        L_array = L.detach().cpu().numpy()
        epsilon = 1e-5
        identity = np.eye(L_array.shape[1], dtype=L_array.dtype)

        temp_list_lamb = []
        for i in range(L_array.shape[0]):
            matrix = L_array[i] + epsilon * identity
            try:
                eigvals = np.linalg.eigvalsh(matrix)
                eigvals = np.real(eigvals)
                lamb = np.sort(eigvals)[-k:]
                ...
                try:
                    ...
                    lamb = np.array([default_value], dtype=np.float32)

            temp_list_lamb.append(float(lamb[-1]))

        return np.asarray(temp_list_lamb, dtype=np.float32)
    ...
    def forward(self, G, Loop_G, RNA_OP, RNA_ELMo, RegionIndex):
        ...
        # print(RNA_OP.shape) # torch.Size([706, 41, 15])
        # print(RNA_ELMo.shape) # torch.Size([706, 41, 256])
        ...
        return final_feature, scores, Constructive_loss, distance_sums
