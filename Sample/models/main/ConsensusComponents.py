...
一、KeySubsequenceFinder：从RNA序列中找到关键的子区域，并构建motif图
1. 输入为RNA序列、graph,分别从共识基序、环结构等先验知识入手，首先筛选出序列中潜在的关键结构。
2. 此处包含可学习权重形状为两个nx41的权重矩阵，考虑这两个权重矩阵是否需要进一步的优化,这部分的权重放到对比的地方去。
3. 这部分的判断考虑要放到模型之外去，循环判断太花时间了

二、ConsensusMoitfAware: 基于结构感知获取共识知识向量，并构建共识图。
（1）以聚类方式动态更新锚点集。
（2）基于获得的锚点集定义共识，即所有其他节点距离该锚点集的总距离应该越小越好，表明该锚点集更具共识性。
（3）这部分只引入了一个长度为n的可学习权重，这部分的损失函数可能需要重新设计下
（4）计算余弦相似性，构建共识图。共识图与原始图像相加，得到最终的共识graph。

四、ContrastiveRepresention: 基于结构感知获取共识知识。
（1）以关键子区域和非关键子区域分别作为正负样本进行对比。对比时引入一中的一级结构权重和二级结构权重。执行以下内容的对比：
    a, 正样本的：
（2）融合
...
class SequenceSplitter(nn.Module):
    ...
    def forward(self, C, L):
        ...
        self._initial_Pos_weights(C.size(0), L.size(1))
        #if self.Neg_weights is None:
        self._initial_Neg_weights(C.size(0), L.size(1))
        ...
        # print(neg_weights_mask.shape)
        # print(pos_weights_mask.shape)
        # print(C.shape)
...
class NodeConsensusVector(nn.Module):
    ...
    def forward(self, motifGraph):
        ...
        active_anchor_num = max(1, len(anchor_indices))
        ...
        distance_sums = distance_sums.sum(dim=0) / (num_nodes * active_anchor_num)
       
        # print('self.weight_matrix '+str(self.weight_matrix.shape))
        # print('motifGraph.shape '+str(motifGraph.shape))
        # print('consensus_vectors.shape '+str(consensus_vectors.shape))
        weighted_consensus_vectors = self.weight_matrix[:motifGraph.size(0), :active_anchor_num] * consensus_vectors  # 这个权重矩阵需要参与优化
