...
#钩子mask
def get_zero_grad_hook(mask, device=None):
    """zero out gradients"""
    def hook(grad):
        target_device = device if device is not None else grad.device
        return grad * mask.to(target_device)
    return hook
...
def tensor_diag(A):
    ...
    B = torch.zeros((rows, cols, cols), device=device, dtype=A.dtype)
...
def threshold_filter(adj_tensor):
    ...
    # 将小于行平均值的元素置零
    adj_tensor = torch.where(torch.lt(adj_tensor, row_means), torch.zeros_like(adj_tensor), adj_tensor)
